...
class _HomeWidgetState extends ConsumerState<HomeWidget> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      ref.read(postsProvider.notifier).loadInitialPosts(user?.id, tab: _activeTab.name);
    });
  }
          ...
          onTabChanged: (tab) {
            ...
            if (tab == _activeTab) {
              ...
              if (tab != HomeTab.live) {
                ...
                ref.read(postsProvider.notifier).loadInitialPosts(userId, tab: tab.name);
              }
            } else {
  ...
  Widget _buildFeed(User? user) {
    ...
    return PageView.builder(
      ...
      onPageChanged: (index) {
        ...
        // 🔥 PRÉCHARGE DES MINIATURES UNIQUEMENT
        // On ne précharge pas le .m3u8 avec CacheManager, ça ne sert à rien.
        if (index + 1 < postsState.posts.length) {
          final nextPost = postsState.posts[index + 1];
          
          for (final media in nextPost.medias) {
            print("VIDEO URL: ${media.url}");
            print("THUMB URL: ${media.thumbnailUrl}");
            if (media.thumbnailUrl != null) {
              // On précharge la miniature pour qu'elle soit instantanée
              CachedNetworkImageProvider(media.thumbnailUrl!);
            }
          }
        }
      },
      itemBuilder: (_, index) {
        ...
        return _PostItem(
          ...
          onToggleFollow: () => ref.read(postsProvider.notifier).toggleFollow(post.authorId, user!.id),
          onDoubleTapLike: () => ref.read(postsProvider.notifier).toggleLike(post.id, user!.id, post.authorId),
        );
      },
    );
  }
    ...
    return GestureDetector(
      ...
      child: Stack(
        children: [
          Positioned.fill(
            child: _MediaCarousel(
              ...
              onHorizontalDragStart: () =>
                  widget.onLockVerticalScroll(true),
              onHorizontalDragEnd: () =>
                  widget.onLockVerticalScroll(false),
          ...
          _BottomLeftText(
            title: widget.post.medias.isNotEmpty
...
class _MediaCarouselState extends State<_MediaCarousel> {
  ...
  Widget build(BuildContext context) {
    if (widget.medias.isEmpty) {
      return Container(decoration: const BoxDecoration(gradient: kDarkGradient));
    }

    return NotificationListener<ScrollNotification>(
      onNotification: (notification) {
        if (notification is ScrollStartNotification) widget.onHorizontalDragStart();
        if (notification is ScrollEndNotification) widget.onHorizontalDragEnd();
        return false;
      },
      child: PageView.builder(
        ...
        itemBuilder: (_, i) {
          ...
          final bool isCurrentPage = i == _currentIndex;
          final bool shouldPlay = widget.isPostVisible && isCurrentPage;
          ...
          return Container(
            ...
            child: Stack(
              fit: StackFit.expand,
              children: [
                // 1. MINIATURE (Fond) : S'affiche immédiatement (depuis le cache)
                if (media.thumbnailUrl != null)
                  CachedNetworkImage(
                ...
                else if (media.mediaType == 'image' && media.url != media.thumbnailUrl)
                  // On affiche l'image haute définition par dessus la miniature si elles sont différentes
                  CachedNetworkImage(
                    imageUrl: media.url,
                    fit: BoxFit.cover,
                  ),
...
class _VideoPlayerFullScreenState extends State<VideoPlayerFullScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    if (widget.shouldPlay) _initVideo();
  }

  Future<void> _initVideo() async {
    if (_initialized || _controller != null) return;

    // 🛡️ VERIFICATION : Si l'URL contient "iframe", on sait que ça va échouer
    if (widget.videoUrl.contains('iframe.mediadelivery.net')) {
      debugPrint("⚠️ ERREUR : Tentative de lecture d'une iframe au lieu d'un flux direct.");
    ...
    try {
      ...
      /* final controller = VideoPlayerController.networkUrl(
        Uri.parse("https://vz-920fd3ca-550.b-cdn.net/f89d1136-ae68-4a6d-bce3-0cb3ce0da765/playlist.m3u8"),
      ); */
  ...
  void _videoListener() {
    ...
    // Logique de détection de vue (3 secondes)
    if (_controller!.value.position >= const Duration(seconds: 3) && !_viewSent) {
      _viewSent = true;
      final watchTime = _controller!.value.position.inSeconds - _startWatchSecond;
      widget.onViewThresholdReached?.call(watchTime > 0 ? watchTime : 3);
    }
  }
  ...
  Widget build(BuildContext context) {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const Text("Erreur de connexion", style: TextStyle(color: Colors.white)),
            TextButton(
              onPressed: () {
    ...
    // Si pas encore chargé, on laisse voir la miniature du Stack parent
    ...
    return GestureDetector(
      onTap: () {
        setState(() {
          ...
          widget.onPlayingChanged?.call(_controller!.value.isPlaying);
        });
      },
      child: Stack(
        ...
        ActionIconCommentaire(
          ...
          onOpenComments: () {
            showModalBottomSheet(
              context: context,
              isScrollControlled: true, // INDISPENSABLE pour le DraggableScrollableSheet
              backgroundColor: Colors.transparent, // Permet de voir l'arrondi et l'ombre
              barrierColor: Colors.black54, // Assombrit légèrement l'arrière-plan du post
